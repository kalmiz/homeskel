...
function! s:SnipplrList(bang,...)
  let cmd = "ruby1.9 " . g:snipplr_rb . " -l "
  if a:bang == 1
    let cmd = cmd . " --nocache"
  endif
...
function! s:SnipplrGet(query, bang)
  ...
  let cmd = "ruby1.9 " . g:snipplr_rb . ' -g ' . a:query
  if a:bang == 1
    let cmd = cmd . " --nocache"
  endif
...
function! s:SnipplrInfo(snippet_id)
  let tempfile = tempname()
  let cmd = "ruby1.9 " . g:snipplr_rb . ' -i ' . a:snippet_id
  call writefile(split(system(cmd), "\n"), tempfile)
  belowright split 
  exec "edit " . tempfile
  nnoremap <buffer> <silent> c :<C-u>bdelete<CR>
...
function! s:SnipplrDelete(snippet_id)
  let cmd = "ruby1.9 " . g:snipplr_rb . ' -d ' . a:snippet_id
  call system(cmd)
  if v:shell_error != 0
    echo "error"
  else
